...
class tea_list(APIView):
    def get(self,request,store_id):
        ...
        for tea in teas:
            context["teas"][tea.id]={"id":tea.id, "name":tea.name, "short_intro":tea.details, "picture":tea.picture.url}
        return HttpResponse(json.dumps(context), content_type="json", charset='utf-8')
...
class tea_detail(APIView):
    def get(self,request, tea_id):
        ...
        #context = {'tea': tea}
        context = {"ID":tea.id,"name":tea.name,"store":tea.store.name, "picture":tea.picture.url, "details":tea.details, "TeaAdds":{}}
